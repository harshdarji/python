...
for n in range(1,4475):
    ...
    for l in lines:
        ...
        t=t+l
    thisScore={"name":file[:5], "values":[0,0,0,0,0,0], "size":len(t)}
    type='none'
    typeMax=0
    for word in placesW:
    ...
    for word in bioW:
        thisScore["values"][2]=thisScore["values"][2]+count_words(t,word)
        replace(word, 'bio')
    if thisScore["values"][2]>typeMax:
        type='bio'
        typeMax=thisScore["values"][2]
    for word in airW:
        thisScore["values"][3]=thisScore["values"][3]+count_words(t,word)
        replace(word, 'air')
    if thisScore["values"][3]>typeMax:
        type='air'
        typeMax=thisScore["values"][3]
    for word in fireW:
        thisScore["values"][4]=thisScore["values"][4]+count_words(t,word)
        replace(word, 'fire')
    if thisScore["values"][4]>typeMax:
        type='fire'
        typeMax=thisScore["values"][4]
    for word in hackW:
        thisScore["values"][5]=thisScore["values"][5]+count_words(t,word)
        replace(word, 'hack')
