...
out=open("C:\\Documents and Settings\\cukier_j\\My Documents\\vast challenge\\MC_1_Materials_3-30-2011\\ftw.js",'w')
rownum = 0
interestingTweets=[]
...
for period in range(21*4):
    ...
    for col in range(xMax):
        ...
        for row in range(yMax):
            ebbflow[period][col].append({'nb':0,'score':0, 'tweets':[]})
...
for row in reader:
    ...
    if rownum == 0:
        ...
        ebbflow[tweetTimeIndex][tweetCol][tweetRow]["nb"]=ebbflow[tweetTimeIndex][tweetCol][tweetRow]["nb"]+1
        ebbflow[tweetTimeIndex][tweetCol][tweetRow]["score"]=ebbflow[tweetTimeIndex][tweetCol][tweetRow]["score"]+score
        ebbflow[tweetTimeIndex][tweetCol][tweetRow]["tweets"].append([row[1].split(' ')[1], row[3]])
